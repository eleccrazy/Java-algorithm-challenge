/**
 * 2. A prime number is an integer that is divisible only by 1 and itself. A
 * porcupine number is a prime number whose last digit is 9
 * and the next prime number that follows it also ends with the digit 9. For
 * example 139 is a porcupine number because:
 * a. it is prime
 * b. it ends in a 9
 * c. The next prime number after it is 149 which also ends in 9. Note that 140,
 * 141, 142, 143, 144, 145, 146, 147 and 148 are not
 ...
 * and returns the first porcupine number that is
 * greater than n. So findPorcupineNumber(0) would return 139 (because 139
 * happens to be the first porcupine number) and so would
 ...
 * is prime, otherwise it returns 0. E.G., isPrime(7)
